...
function drawExitConfirm() {
    if (!staticDrawn || exitConfirmSelection !== menuLastSelection) {
        fillScreen(BLACK); 
        setTextSize(2);
        setTextColor(YELLOW);
        ...
        staticDrawn = true;
        menuLastSelection = exitConfirmSelection;
    }
}
var PADDLE_WIDTH = 40;
var PADDLE_HEIGHT = 6;
...
var slotWeights = [10, 17, 17, 28, 28];
    ...
    switch (flappyState) {
        ...
        case FLAPPY_STATE_GAME:
            ...
            for (var i = 0; i < pipes.length; i++) {
                ...
                drawFillRect(pipes[i].x, 0, clearWidth, pipes[i].y, LIGHTBLUE);
                drawFillRect(pipes[i].x, pipes[i].y + pipes[i].height, clearWidth, HEIGHT - pipes[i].y - pipes[i].height - GROUND_HEIGHT, LIGHTBLUE);
            }
            drawFillRect(WIDTH / 2 - 15, 20, 30, 24, LIGHTBLUE);
            setTextSize(2);
            setTextColor(WHITE);
            ...
            drawFillRect(bird.x - bird.width / 2, bird.lastY - bird.height / 2, bird.width, bird.height, LIGHTBLUE);
...
function drawBlackjack() {
    if (!staticDrawn || blackjack.state !== drawBlackjack.lastState || 
        ...
        if (blackjack.state === 0) {
            ...
            var nextHandX = 120 - (nextHandText.length * 5) / 2; // Center at 120, ~5px per char for size 1
...
function handleInput() {
    ...
    switch (gameState) {
        ...
        case STATE_SNAKE:
            ...
            if (prevPressed && snakeState === SNAKE_STATE_GAME && snakeCanMove) {
                nextDirection = (direction - 1 + 4) % 4;
                snakeCanMove = false;
            }
            if (nextPressed && snakeState === SNAKE_STATE_GAME && snakeCanMove) {
        ...
        case STATE_EXIT_CONFIRM:
            ...
            if (currentSelState && !menuLastSelState) {
                if (exitConfirmSelection === 0) {
                    ...
                    if (prevGameState === STATE_SLOTS) {
                        gameState = STATE_SLOTS;
                        slotState = SLOT_STATE_PAUSED;
                        slotStaticDrawn = false;
                    } else if (prevGameState === STATE_FLAPPY_BIRD) {
                        if (gameState === STATE_PAUSED && flappyState === FLAPPY_STATE_PAUSED) {
                            gameState = STATE_PAUSED;
                            flappyState = FLAPPY_STATE_PAUSED;
                            pauseStaticDrawn = false;
                        } else {
                            gameState = STATE_GAME_OVER;
                            suppressGameOverSound = true;
                            flappyState = FLAPPY_STATE_MENU;
                            staticDrawn = false;
                        }
                    } else if (prevGameState === STATE_BLACKJACK) {
                        gameState = STATE_BLACKJACK;
                        blackjack.state = 4;
                        staticDrawn = false;
                        pauseMenuSelection = 0;
                    } else {
                        gameState = prevGameState === -1 ? STATE_MAIN_MENU : prevGameState;
                        staticDrawn = false;
                    }
                    mainMenuStaticDrawn = false;
                    menuLastSelState = false;
